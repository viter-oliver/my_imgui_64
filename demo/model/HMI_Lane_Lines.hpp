...
#ifndef __HMI_Lane_Lines_hpp__
#define __HMI_Lane_Lines_hpp__

#include "HMI_Lane_Line.hpp"
#include "HMI_Lane_Line.hpp"
#include "HMI_Lane_Line.hpp"
#include "HMI_Lane_Line.hpp"
...
{
    public:
        HMI_Lane_Line left_line;

        HMI_Lane_Line right_line;

        HMI_Lane_Line next_left_line;

        HMI_Lane_Line next_right_line;
...
uint64_t HMI_Lane_Lines::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == HMI_Lane_Lines::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)HMI_Lane_Lines::getHash };

    uint64_t hash = 0x512788b262e951e1LL +
         HMI_Lane_Line::_computeHash(&cp) +
         HMI_Lane_Line::_computeHash(&cp) +
         HMI_Lane_Line::_computeHash(&cp) +
         HMI_Lane_Line::_computeHash(&cp);
