#include "ft_rotate_twist_curve_3d.h"
#include "ft_light_scene.h"
#include "ft_trans.h"
#include "ft_raw_trans.h"

static const char* sd_rt_tw_curve_vs = R"glsl(#version 300 es
...
const float right_a=1.5707963;//pi*0.5
void main()
{
    vec3 pos=position;
    float posx0=pos.x;
    ...
    if(ori_x>0.1){//right rotate
        ...
        if( z<=ori_z||r_s<=u_l) {
            ...
            if(posx0<0.1){//left
              float tra_up=tra*0.5;
              pos.y=w*sin(tra_up);
              float pj_s=w*cos(tra_up);
              float in_x=rd_in*(1.0f-cos(tra));
              float in_z=rd_in*sin(tra)+ori_z;
              pos.x=in_x-pj_s*cos(tra);
              pos.z=in_z+pj_s*sin(tra);
            } else {
            ...
            if(posx0<0.1){//left
               ...
               pos.y=w*sin(tra_up);
               float pj_s=w*cos(tra_up);
               float in_x=rd_in*(1.0f-cos(ra));
               float in_z=rd_in*sin(ra)+ori_z;
               float pos_out_x0=in_x-pj_s*cos(ra);
               float pos_out_z0=in_z+pj_s*sin(ra);
               pos.x=pos_out_x0+dx;
               pos.z=pos_out_z0+dz;
            } else {
...
static const char* sd_rt_tw_curve_fs = R"glsl(#version 300 es
precision highp float;
in vec2 TextCoord;
out vec4 o_clr;
...
{
     ps_shader ft_rotate_twist_curve_3d::_phud_sd = nullptr;
     ps_primrive_object ft_rotate_twist_curve_3d::_ps_prm = nullptr;

     ft_rotate_twist_curve_3d::ft_rotate_twist_curve_3d()
     {
          /*if( !_phud_sd )
          {
               _phud_sd = make_shared<af_shader>( sd_rt_tw_curve_vs, sd_rt_tw_curve_fs );
          }
        */
     ...
     void ft_rotate_twist_curve_3d::link()
     {
          ...
          if( !ft_rotate_twist_curve_3d::_phud_sd )
          {
              ft_rotate_twist_curve_3d::_phud_sd = make_shared<af_shader>( sd_rt_tw_curve_vs, sd_rt_tw_curve_fs );
              ft_rotate_twist_curve_3d::_ps_prm = make_shared<primitive_object>();
              int demension = 5;
              ...
              ft_rotate_twist_curve_3d::_ps_prm->set_ele_format( { 3, 2 } );
              ft_rotate_twist_curve_3d::_ps_prm->load_vertex_data( vertices, data_cnt );
              delete[] vertices;
          }
     }

     void ft_rotate_twist_curve_3d::draw()
     {
          if( !_pat_image )
          {
               return;
