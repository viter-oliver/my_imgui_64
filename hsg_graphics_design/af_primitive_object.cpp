...
void primitive_object::load_vertex_data(GLfloat *pvertex_data, GLuint vetexlen,
                                        GLuint *pele_buff, GLuint ele_cnt,
                                        GLuint mem_usage) {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * _vertex_buf_len, pvertex_data,
               mem_usage);

  if (pele_buff) {
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * _ele_buf_len,
                 pele_buff, mem_usage);
  }
  ...
  for (auto &el : _ele_format) {
    ...
    glVertexAttribPointer(idx, el, GL_FLOAT, GL_FALSE, stride * sizeof(GLfloat),
...
static internal_prm internal_primitive_name_list[] = {
    {"cube",
     ...
     _indices,
     sizeof(_indices) / sizeof(GLuint)},
    {"plain",
     {3, 2},
     _plain_vertices,
     sizeof(_plain_vertices) / sizeof(GLfloat),
     0,
     0},
};
bool ref_a_intenal_primitive(string &prm_name) {
  ...
  for (int ix = 0; ix < isz; ix++) {
    ...
    if (prm_name == prm_un._name) {
      ...
      ps_prm->load_vertex_data(prm_un._pvert, prm_un._vert_cnt, prm_un._ele,
                               prm_un._ele_cnt);
      auto buff_len = 4 + prm_un._vert_cnt * sizeof(float) +
                      prm_un._ele_cnt * sizeof(GLuint);
...
bool get_prm_data(const char *prm_name, float *pbuff, uint32_t buff_len) {
  ...
  phead += 4;
  assert(*phead_buff_len == buff_len * sizeof(float) &&
         "cannot get primitive data for invalid buff_len");
...
bool update_prm_vbo(const char *prm_name, float *pvertex, uint32_t buff_len) {
  ...
  assert(buff_len == prm_obj._vertex_buf_len &&
         "cannot update vbo for invalid buff_len");
  glBindBuffer(GL_ARRAY_BUFFER, prm_obj._vbo);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * prm_obj._vertex_buf_len,
               pvertex, GL_DYNAMIC_DRAW);
  return true;
}