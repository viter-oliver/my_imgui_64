...
void primitve_edit::draw_primitive_list() {
  ...
  if (ImGui::BeginPopupModal("NewPrimitive")) {
    ...
    ImGui::InputText("enter a name for the new primitive object", prm_name_str,
                     FILE_NAME_LEN);
    ImGui::InputInt(
        "enter a number for the count of vertex of new primitive object",
        &vetex_cnt);
    ImGui::Combo("memory usage", &mem_usage_idx, &get_mem_usage_item,
                 mem_usage_item, en_mem_usage_cnt);
  ...
  if (ImGui::Button("Import primitive...")) {
    ...
    const char *prm_format = "ALL Files(*.*)\0*.*\0"
                             "prm\0*.prm\0";
    ...
    ofn.lpstrTitle = "select a primitive file please!";
  ...
  if (ImGui::BeginPopupContextWindow()) {
    if (_pmobj &&
        ImGui::MenuItem("delete", NULL, false, _pmobj.use_count() == 2)) {
...
void primitve_edit::draw_primitive_item_property() {
  if (_pmobj) {
    ImGui::Text("Vertex buffer length:%d ,element buffer length:%d",
                _pmobj->_vertex_buf_len, _pmobj->_ele_buf_len);
    ...
    if (ImGui::BeginPopupModal("Vertex edit")) {
      ...
      if (ImGui::Button("Save")) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * _pmobj->_vertex_buf_len,
                     _pvertex, mem_usage);
        ImGui::CloseCurrentPopup();
