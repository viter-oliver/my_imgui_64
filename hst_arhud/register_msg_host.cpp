...
bool be_triger_ani()
{
    return pnavi_ani_curl->is_visible();
}
void cancel_navi_ani()
{
    if (pnavi_ani_curl->is_visible()) {
        cancel_play_tran("navi_ani_curl");
        pnavi_ani_curl->set_visible(false);
    }
}
...
void trig_ani()
{
    if (keep_straight_cnt > 0) {
        ...
        if (keep_straight_cnt == 0) {
            ...
            pnavi_ani_curl_wall->set_rotate_angle(angle_fix)
                .set_alpha(alpha);
            printf("angle fix: %f\n", angle_fix);
        }
    }
    ...
    if (!be_triger_ani()) {
        if (will_triger_ani()) {
            ...
            if (is_triggered) {
                ...
                pnavi_ani_curl_wall->set_rotate_angle(rotate_angle_max)
                    .set_alpha(1.f);
                steering_angle_fix = rotate_angle_max;
...
{
    const s16 max_acc = 20, angle_limit = 25;
...
void reg_debug()
{
    ...
    fifo_debuger::attach_var_handle("capture_video", [](char *pcmd_buff) -> bool {
        ...
        if (hud::capture_video) {
            hud::pleft_lane->set_visible(true);
            hud::pright_lane->set_visible(true);
            hud::pleft_lane->set_lane_color(col_g);
            hud::pright_lane->set_lane_color(col_g);
            *hud::pprojector->get_view_pos() = {-0.37f,1.18f,-2.37f};
        } else {
            hud::pleft_lane->set_visible(false);
            hud::pright_lane->set_visible(false);
    ...
    fifo_debuger::attach_var_handle("play_on", [](char *pcmd_buff) -> bool {
        int play_flag = atoi(pcmd_buff);
        float posy = 1580;
        if (play_flag == true) {
            play_tran("play_text", 0, 1, 1000);
        } else {
            cancel_play_tran("play_text");
            set_property_aliase_value_T("hst_text_pos_x", posy);
        }
        return true;
        });
    fifo_debuger::attach_var_handle("pdes_on", [](char *pcmd_buff) -> bool {
        int play_flag = atoi(pcmd_buff);
        auto ptr = static_cast<ft_rectangle_3d *>(hud::pdes->get_child(0));
        if (play_flag == true) {
            hud::pdes->set_translation_z(24.863)
                .set_translation_x(0.584)
                .set_visible(true);
            ptr->set_model_col({1.f,0,0});

        } else {
            hud::pdes->set_visible(false);
            ptr->set_model_col({1.f,0.455,0});
        }
        return true;
        });
}
...
#define CUR_DE_OFFSET 0.00012206658721
        ...
        mh.register_msg_handle(AEB_WARNING, [&](u8 *pbuff, int len) {
            ...
            if (hud::capture_video) return false;
            ...
            if (pcan->FCW_ON) {
                printf("fcw_on~~~~~~~~~~~~\n");
                auto prt = static_cast<ft_rectangle_3d *>(hud::pmain_tar->get_child(0));
            ...
            hud::pright_lane->set_visible(pcan->Right_LDW_ON);
        ...
        mh.register_msg_handle(VechicleSpeedDisplay,
            [&](u8 *pbuff, int len) {
                ...
                if (!km_speed) {
                    ...
                    // cancel animetion & reset position when vspeed beyond 0 km/h
                    cancel_play_tran("play_text");
                    set_property_aliase_value_T("hst_text_pos_x", 1580.f);
                }
                return true;
            });
