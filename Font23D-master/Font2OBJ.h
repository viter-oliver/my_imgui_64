...
std::vector<p2t::Point *> triangulateContour(Vectoriser *vectoriser, int c,
                                             float offset) {
...
Vectoriser *getVectoriser(FT_Face face, wchar_t ch, unsigned short bezierSteps,
                          double strength) {
  ...
  if (strength != 0.0) {
    ...
    FT_Outline_Translate(&face->glyph->outline, -strength * 64.0 * 0.5,
                         -strength * 64.0 * 0.5);
  }
...
float AddCharacter(Mesh &mesh, FT_Face face, char32_t ch,
                   unsigned short bezierSteps, float offset, float extrude,
                   float bevelRadius, unsigned short bevelSegments) {
  ...
  for (size_t c = 0; c < vectoriser->ContourCount(); ++c) {
    ...
    if (contour->GetDirection()) {
      std::vector<p2t::Point *> polyline =
          triangulateContour(vectoriser, c, offset);
      ...
      for (size_t cm = 0; cm < vectoriser->ContourCount(); ++cm) {
        ...
        if (c != cm && !sm->GetDirection() && sm->IsInside(contour)) {
          std::vector<p2t::Point *> pl =
              triangulateContour(vectoriser, cm, offset);
          cdt->AddHole(pl);
        }
      }
  ...
  for (int i = 0; i < bevelSegments; i++) {
    double bevelX =
        bevelRadius * 1.5 * sinf(M_PI * 0.5 * i / (double)bevelSegments);
    double nextBevelX =
        bevelRadius * 1.5 * sinf(M_PI * 0.5 * (i + 1) / (double)bevelSegments);
    ...
    for (size_t c = 0; c < vectoriser1->ContourCount(); c++) {
      ...
      for (size_t p = 0; p < contour1->PointCount(); p++) {
        ...
        ftgl::Point d2 = contour1->GetPoint(
            (p == contour1->PointCount() - 1) ? (0) : (p + 1));
        d1 = d1 * (1.0f / 64.0f);
        d2 = d2 * (1.0f / 64.0f);

        ftgl::Point d3 = contour2->GetPoint(p);
        ftgl::Point d4 = contour2->GetPoint(
            (p == contour2->PointCount() - 1) ? (0) : (p + 1));
        d3 = d3 * (1.0f / 64.0f);
        d4 = d4 * (1.0f / 64.0f);

        double bevelY =
            extrude +
            (bevelRadius * cosf(M_PI * 0.5 * (i) / (double)bevelSegments));
        double nextBevelY =
            extrude +
            (bevelRadius * cosf(M_PI * 0.5 * (i + 1) / (double)bevelSegments));
  ...
  for (size_t c = 0; c < vectoriser->ContourCount(); ++c) {
    ...
    for (size_t p = 0; p < contour->PointCount(); ++p) {
      ...
      ftgl::Point d2 =
          contour->GetPoint((p == contour->PointCount() - 1) ? (0) : (p + 1));
      d1 = d1 * (1.0f / 64.0f);
      d2 = d2 * (1.0f / 64.0f);
...
Mesh generateMesh(char *fontFile, int height, std::u32string str,
                  unsigned short bezierSteps, float extrude, float bevelRadius,
                  unsigned short bevelSteps) {
  FT_Library library;
  ...
  if (FT_New_Face(library, fontFile, 0, &face))
    printf("FT_New_Face failed (there is probably a problem with your font "
           "file\n");
