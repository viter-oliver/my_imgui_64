...
std::vector<Device> DeviceEnumerator::getVideoDevicesMap() {
	return getDevicesMap(CLSID_VideoInputDeviceCategory);
}

std::vector<Device> DeviceEnumerator::getAudioDevicesMap() {
	return getDevicesMap(CLSID_AudioInputDeviceCategory);
}

// Returns a map of id and devices that can be used
std::vector<Device> DeviceEnumerator::getDevicesMap(const GUID deviceClass)
{
	std::vector<Device> devices;

	HRESULT hr = CoInitialize(nullptr);
	if (FAILED(hr)) {
		return devices; // Empty devices as an error
	}
	...
	if (SUCCEEDED(hr)) {
		...
		while (pEnum->Next(1, &pMoniker, NULL) == S_OK) {
			...
			// We populate the map
			devices.push_back({ devicePath,deviceName });
		}
		pEnum->Release();
	}
	CoUninitialize();
