...
using namespace std;
/**
 * @brief  primitive_object represent basic data struct which will be inputed
 * into\n a vertex shader
 */
enum {
  en_gl_points,
...
struct primitive_object {
  ...
  primitive_object() : _vertex_buf_len(0), _ele_buf_len(0) {
  ...
  void set_ele_format(vector<GLubyte> ele_fm) { _ele_format = ele_fm; }
  void load_vertex_data(GLfloat *pvertex_data, GLuint vetexlen,
                        GLuint *pele_buff = 0, GLuint ele_cnt = 0,
...
/** the primitive object of g_primitive_list will be referenced by some ui
 * components */
...
extern bool AFG_EXPORT get_prm_data(const char *prm_name, float *pbuff,
                                    uint32_t buff_len);
extern bool AFG_EXPORT update_prm_vbo(const char *prm_name, float *pvertex,
                                      uint32_t buff_len);